...
namespace Pronia.Areas.ProniaAdmin.Controllers
{
    ...
    public class PlantController : Controller
    {
        ...
        public async Task<IActionResult> Edit(int id, Plant plant)
        {
            ...
            List<PlantImage> removeableImages = existed.PlantImages.Where(p => p.IsMain == false && !plant.ImageIds.Contains(p.Id)).ToList();

            List<PlantImage> removeableImage = existed.PlantImages.Where(p => p.IsMain == true && plant.ImageId != p.Id).ToList();

            existed.PlantImages.RemoveAll(p => removeableImage.Any(ri => ri.Id == p.Id));
            ...
            foreach (var image in removeableImage)
            {
